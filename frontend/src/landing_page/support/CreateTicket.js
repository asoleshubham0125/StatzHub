import React from "react";

function CreateTicket() {
  return (
    <div className="container">
      <div className="row p-5 mb-5 mt-5">
        <h1 className="fs-2 text-center">
          To create a ticket, select a relevant topic
        </h1>
        <div className="col-4 p-5 mb-2 mt-2">
          <h4 className="">
            <i class="fa-solid fa-circle-plus"></i>Account Opening{" "}
          </h4>
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Online Account Opening
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Offline Account Opening
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Company, Partnership and HUF Account
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Opening
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            NRI Account Opening
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Charges at StatzHub
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            StatzHub IDFC FIRST Bank 3-in-1 Account
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Getting Started
          </a>
          <br />
        </div>
        <div className="col-4 p-5 mt-2 mb-2">
          <h4 className="">
            <i class="fa-solid fa-user"></i> Your StatzHub Account
          </h4>
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Login Credentials
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Account Modification and Segment Addition
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Your Profile
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            DP ID and bank details
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Transfer and conversion of shares
          </a>
          <br />
        </div>
        <div className="col-4 p-5 mb-2 mt-2">
          <h4 className="">
            <i class="fa-regular fa-circle"></i> Orbit
          </h4>
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            IPO
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Trading FAQs
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Margin Trading Facility (MTF) and Margins
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Charts and orders
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Alerts and Nudges
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            General
          </a>
          <br />
        </div>
        <div className="col-4 p-5 mb-2">
          <h4 className="">
            <i class="fa-solid fa-indian-rupee-sign"></i> Funds
          </h4>
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Add money
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Withdraw money
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Add bank accounts
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            eMandates
          </a>
          <br />
        </div>
        <div className="col-4 p-5 mb-2 mt-2">
          <h4 className="">
            <i class="fa-solid fa-terminal"></i> Console
          </h4>
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Portfolio
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Corporate actions
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Funds statement
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Reports
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Profile
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Segments
          </a>
          <br />
        </div>
        <div className="col-4 p-5 mb-2 mt-2">
          <h4 className="">
            <i class="fa-brands fa-contao"></i> Coin
          </h4>
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Mutual funds
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            National Pension Scheme (NPS)
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Features on Coin
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            Payments and Orders
          </a>
          <br />
          <a
            className="custom-link"
            href=""
            style={{ textDecoration: "none", lineHeight: "2.5" }}
          >
            General
          </a>
          <br />
        </div>
      </div>
    </div>
  );
}

export default CreateTicket;
