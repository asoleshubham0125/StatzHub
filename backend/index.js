...
app.post("/newOrder", async (req, res) => {
  try {
    ...
    if (mode === "BUY") {
      ...
      return res.send("BUY order saved → New lot created");
    }

    if (mode === "SELL") {
      const lots = await HoldingsModel.find({ name }).sort({ createdAt: 1 });

      const totalQty = lots.reduce((sum, lot) => sum + lot.qty, 0);

      if (totalQty < qty) {
        return res.status(400).send("Sell failed: Not enough quantity");
      }
      ...
      for (const lot of lots) {
        ...
        if (lot.qty <= remaining) {
          remaining -= lot.qty;
          await HoldingsModel.deleteOne({ _id: lot._id });
        } else {
      ...
      return res.send("SELL order saved → Lot quantities updated (FIFO)");
    }

    res.status(400).send("Invalid order mode");
  } catch (err) {
    console.log(err);
    res.status(500).send("Error processing new order");
  }
});
